...
describe('Record', function () {
    it('defines a constructor', function () {
        var MyType = immutable_1.Record({ a: 1, b: 2, c: 3 });
        var t1 = new MyType();
        var t2 = t1.set('a', 10);
        var t3 = t2.clear();
        expect(t1 instanceof immutable_1.Record).toBe(true);
        expect(t1 instanceof MyType).toBe(true);
        expect(t3 instanceof immutable_1.Record).toBe(true);
    ...
    it('passes through records of the same type', function () {
        var P2 = immutable_1.Record({ x: 0, y: 0 });
        var P3 = immutable_1.Record({ x: 0, y: 0, z: 0 });
        var p2 = P2();
        var p3 = P3();
    ...
    it('converts sequences to records', function () {
        var MyType = immutable_1.Record({ a: 1, b: 2, c: 3 });
        var seq = immutable_1.Seq({ a: 10, b: 20 });
    ...
    it('skips unknown keys', function () {
        var MyType = immutable_1.Record({ a: 1, b: 2 });
        var seq = immutable_1.Seq({ b: 20, c: 30 });
        var t = new MyType(seq);
